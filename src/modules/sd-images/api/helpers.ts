...
export const getParamsFromPrompt = (originalPrompt: string, model?: IModel): IParams => {
  ...
  if (prompt.includes('Negative prompt:')) {
    const param_blob = prompt.split('Negative prompt: ')[1] || ''
    const c_prompt = prompt.split('Negative prompt: ')[0].trim() || ''
    const c_negativePrompt = param_blob.split('Steps: ')[0].trim() || ''
    const step_match = param_blob.match(/Steps:\s+(\d+)/)
    const c_steps = step_match ? step_match[1] : '20'
    const dim_match = param_blob.match(/Size:\s+(\d+x\d+)/)
    const c_dimensions = dim_match ? dim_match[1] : '512x512'
    const cfg_match = param_blob.match(/CFG scale:\s+(\d+(\.\d+)?)/)
    const c_cfg = cfg_match ? cfg_match[1] : '7'
    const seed_match = param_blob.match(/Seed:\s+(\d+)/)
    const c_seed = seed_match ? seed_match[1] : '1234567890'

    const sampler_match = param_blob.match(/Sampler:\s(.*?),\s/)
