...
function findFinder (b: Bitmap) {
  ...
  function checkLine (center: Point, maxCount: number, total: number, incr: Point) {
    ...
    const i = FINDER.check(b, runs, center, incr, maxCount)
  ...
  function check (runs: Runs, i: number, j: number) {
    ...
    y += i
    // Horizontal
    const xx = checkLine({ x: int(x), y: int(y) }, runs[2], total, { y: 0, x: 1 })
...
function findAlignment (b: Bitmap, est: Pattern, allowanceFactor: number) {
  ...
  if (x < moduleSize * 3 || y < moduleSize * 3) { throw new Error(`x = ${x}, y=${y} moduleSize = ${moduleSize}`) }
  const xStart = leftX
  const yStart = topY
...
function _single (b: Bitmap, from: Point, to: Point) {
  ...
  let error = -d.x / 2
  const step = { x: from.x >= to.x ? -1 : 1, y: from.y >= to.y ? -1 : 1 }
  let runPos = 0
  const xLimit = to.x + step.x
...
function readInfoBits (b: Bitmap) {
  ...
  // Version information
  let version1 = 0
  for (let y = 5; y >= 0; y--) { for (let x = size - 9; x >= size - 11; x--) version1 = readBit(x, y, version1) }
  let version2 = 0
  for (let x = 5; x >= 0; x--) { for (let y = size - 9; y >= size - 11; y--) version2 = readBit(x, y, version2) }
  // Format information
  let format1 = 0
...
function decodeBitmap (b: Bitmap) {
  const size = b.height
  if (size < 21 || (size & 0b11) !== 1 || size !== b.width) { throw new Error(`decode: wrong size=${size}`) }
...
export default function readQR (img: Image, opts: DecodeOpts = {}): string {
  for (const field of ['height', 'width'] as const) {
    if (!Number.isSafeInteger(img[field]) || img[field] <= 0) { throw new Error(`Wrong img.${field}=${img[field]} (${typeof img[field]})`) }
  }
  if (
  ...
  if (opts.isRGB !== undefined && typeof opts.isRGB !== 'boolean') { throw new Error(`Wrong opts.isRGB=${opts.isRGB}  (${typeof opts.isRGB})`) }
  for (const fn of ['detectFn', 'qrFn'] as const) {
    if (opts[fn] !== undefined && typeof opts[fn] !== 'function') { throw new Error(`Wrong opts.${fn}=${opts[fn]} (${typeof opts[fn]})`) }
  }
