...
const PATTERNS: ReadonlyArray<(x: number, y: number) => boolean> = [
  (x, y) => (x + y) % 2 == 0,
  (x, y) => y % 2 == 0,
  (x, y) => x % 3 == 0,
  (x, y) => (x + y) % 3 == 0,
...
function penalty (bm: Bitmap) {
  ...
  const sameColor = (row: DrawValue[]) => {
    let res = 0
    for (let i = 0, same = 1, last; i < row.length; i++) {
      if (last === row[i]) {
        same++
...
function drawQRBest (ver: Version, ecc: ErrorCorrection, data: Uint8Array, maskIdx?: Mask) {
  if (maskIdx === undefined) {
    ...
    for (let mask = 0; mask < PATTERNS.length; mask++) { bestMask.add(penalty(drawQR(ver, ecc, data, mask as Mask, true)), mask as Mask) }
